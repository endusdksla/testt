#!/usr/bin/env python3
"""stdio <-> Streamable-HTTP MCP bridge for the guest IDA Pro MCP (zeromcp).

Why this exists: the guest IDA plugin (modular ida-pro-mcp / zeromcp) serves MCP
directly over HTTP at http://<GUEST>:13337/mcp. Pointing Claude Code / Codex at that
URL directly does NOT work on this host: their node/electron network stack gets
EHOSTUNREACH to the VMware host-only guest (only curl/python can reach it). radare2's
MCP works because it is a *local* python stdio server that does the VM HTTP itself.

This bridge does the same for IDA: the AI client launches it over stdio; this process
(python, which CAN reach the guest) transparently relays MCP JSON-RPC messages to the
guest's /mcp endpoint, carrying the Mcp-Session-Id. No extra deps — stdlib only.

Register as a stdio MCP server:
  command = <malware venv python>, args = [<this file>]
Optional arg/env override of the target URL:
  args = [<this file>, "http://172.16.217.129:13337/mcp"]
  or env IDA_MCP_URL=...
"""
import json
import os
...
TARGET = (
    ...
    else os.environ.get("IDA_MCP_URL", "http://172.16.217.129:13337/mcp")
)
...
def _log(msg):
    ...
    print(f"[ida-bridge] {msg}", file=sys.stderr, flush=True)
