    ...
    def handle_mcp(self, method, params):
        ...
        if method == 'analyze':
            ...
            try:
                ...
                if stdout.strip():
                    data = json.loads(stdout)
                    return data.get('imports', data) if isinstance(data, dict) else data
                return []
            except Exception as e:
                return {'error': str(e)}
            ...
            if not Radare2Handler.current_binary:
                return {'error': 'No binary loaded'}
            try:
                cmd = f'rabin2.exe -Ej "{Radare2Handler.current_binary}"'
                result = subprocess.run(cmd, shell=True, capture_output=True, text=True, timeout=60)
                stdout = result.stdout or ''
                if stdout.strip():
                    data = json.loads(stdout)
                    return data.get('exports', data) if isinstance(data, dict) else data
                return []
            except Exception as e:
                return {'error': str(e)}
